...
from book.Chapter5.DistanceMetrics import InstanceDistanceMetrics
import sklearn
import pandas as pd
import numpy as np
from sklearn.metrics import silhouette_samples, silhouette_score
from book.Chapter5.DistanceMetrics import PersonDistanceMetricsNoOrdering
from book.Chapter5.DistanceMetrics import PersonDistanceMetricsOrdering
import random
...
class NonHierarchicalClustering:
    ...
    def aggregate_datasets(self, datasets, cols, abstraction_method):
        ...
        # Flatten all raw_datasets and add them to the newly formed dataset.
    ...
    # We have raw_datasets covering multiple persons. We abstract the datatasets using an approach and create
    # clusters of persons.
    def k_means_over_datasets(self, datasets, cols, k, abstraction_method, distance_metric, max_iters, n_inits, p=1):
        # Convert the raw_datasets to instances
    ...
    # For k-medoids we use all possible distance metrics between raw_datasets as well. For this we
    # again need to define a distance matrix between the raw_datasets.
    ...
    def k_medoids_over_datasets(self, datasets, cols, k, abstraction_method, distance_metric, max_iters, n_inits=5, p=1, max_lag=5):
        ...
        # If we compare raw_datasets by flattening them, we can simply flatten the dataset and apply the instance based
        # variant.
        ...
        # For the case over raw_datasets we do not have a quality metric, therefore we just look at a single initialization for now (!)
        ...
        # Silhouette cannot be used here as it used a distance between instances, not raw_datasets.
...
class HierarchicalClustering:
    ...
    # Perform agglomerative clustering over the raw_datasets by flattening them into a single dataset.

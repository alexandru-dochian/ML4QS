...
DATASET_PATH = Path('../raw_datasets/crowdsignals/csv-participant-one/')
...
for milliseconds_per_instance in GRANULARITIES:
    print(
        f'Creating numerical raw_datasets from files in {DATASET_PATH} using granularity {milliseconds_per_instance}.')
    ...
    dataset.add_event_dataset(
        'labels.csv', 'label_start', 'label_end', 'label', 'binary')

    # We add the amount of light sensed by the phone (continuous numerical measurements) and aggregate by averaging
    dataset.add_numerical_dataset('light_phone.csv', 'timestamps', [
    ...
    DataViz.plot_dataset_boxplot(dataset, [
                                 'acc_phone_x', 'acc_phone_y', 'acc_phone_z', 'acc_watch_x', 'acc_watch_y', 'acc_watch_z'])
...
# Make a table like the one shown in the book, comparing the two raw_datasets produced.
